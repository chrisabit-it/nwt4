...
const int maxmessstep = 600; //Grenze für Progressmessbar Wattmeter
...
const double ydisplayteilung_lin = 11.0; //Displayteilung Y für lin. Messonden
const double yaufloesung = 1023.0;//maximale Auflösung des AD Wandlers
...
//SA
  ...
  double bw300_max;    //Bandweitengrenzen für die automatische Schrittkorrektur
  double bw300_min;    //Bandweitengrenzen für die automatische Schrittkorrektur
  double bw7kHz_max;   //Bandweitengrenzen für die automatische Schrittkorrektur
  double bw7kHz_min;   //Bandweitengrenzen für die automatische Schrittkorrektur
  double bw30kHz_max;  //Bandweitengrenzen für die automatische Schrittkorrektur
  double bw30kHz_min;  //Bandweitengrenzen für die automatische Schrittkorrektur
  ...
  bool bschrittkorr;   //BIT für automatische Schrittkorrektur
  ...
  bool fwfalsch;
  int pfsize;
  bool grperwobbeln;   //BIT für Focusumschaltung
  bool grpereinmal;    //BIT für Focusumschaltung
  bool grperstop;      //BIT für Focusumschaltung
  ...
  int audioztime;      //neu FW120 NF-Audio Zeit für einen Wobbeldurchlauf
  int audiosens;       //neu FW120 NF-Audio Sensibilität
...
typedef struct {
  ...
  bool bkalibrierswrk1;     //Eichlinie für SWR neu
  bool bkalibrierswrk2;     //Eichlinie für SWR neu
  int swraufloesung;   //Auflösung im SWR fenster
  bool bswriteration;  //math. Korrektur der SWR-Kurve
  bool bswrrelais;     //SWR-Kurve
  bool bswrkanal2;     //SWR mit Kanal 2 ermitteln
  double a_100;        //Kabeldämpfung auf 100m
  double kabellaenge;  //Kabellaenge
  bool mousesperre;    //sperren der Mouseaktivitaet
  int kurvennr;        //Nr der Hintergrundkurve nur für WKManager
  QPen penkanal1;      //Stift für Kanal 1
  QPen penkanal2;      //Stift für Kanal 2
  int penwidth;        //Stiftbreite
  bool bhintergrund;   //ob Hintergrund oder nicht für WKManager
