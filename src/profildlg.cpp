...
// Class für Option
//#####################################################################################
profildlg::profildlg(QWidget *parent, const char *name, bool modal) : QDialog(parent) {
  this->setModal(modal);
  ...
  loeschen = new QPushButton(tr("Löschen", "Profil Dialog"), this);
...
QString profildlg::linenormalisieren(const QString &line) {
  ...
  ergebnis *= faktor;
  aline = QString::asprintf("%1.0f", ergebnis);
  return aline;
}
...
void profildlg::tip(bool atip) {
  ...
  QString tip_schritte = profildlg::tr(
      ...
      "1001 Schritte verwendet <br>", "tooltip text");
  ...
  QString tip_name = profildlg::tr(
      ...
      "des Profils. löschen des Profiles <br>"
      "mit \"NN\" oder einfach dieses <br>"
      "Feld löschen. <br>"
      "Dieser String erscheint in der <br>"
      "ComboBox \"Profil\" des Wobbelarbeitsblattes", "tooltip text");
  if (atip) {
    for (i = 0; i < profilanz; i++) {
