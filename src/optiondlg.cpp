...
// Class für Option
//#####################################################################################
odlg::odlg(QWidget *parent, const char *name, bool modal) : QDialog(parent) {
  this->setModal(modal);
  ...
  gkalibrier->setTitle(tr("Dämpfungs-Glied", "Option Dialog"));
  daempfungfa = new QCheckBox(gkalibrier);
  daempfungfa->setGeometry(10, 20, 90, 25);
  daempfungfa->setText(tr("FA-Dämpf ", "Option Dialog"));
  //:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  geinkanalig = new QGroupBox(wwobbel);
  geinkanalig->setGeometry(290, 70, 140, 60);
  geinkanalig->setTitle(tr("Kanäle", "Option Dialog"));
...
//#############################################################################
//#############################################################################
//Grundaten für den Spekrumanalyser
//#############################################################################
//#############################################################################
  ...
  boxsastatus->setText(tr("HW-Rückmeldung vom SA", "Option Dialog"));
  ...
  gpegelkorr->setTitle(tr("Pegelkorr. für dBm-Anzeige", "Option Dialog"));
  ...
  groupsavgrenzen = new QGroupBox(tr("Frequenzen für automatische Schrittkorrektur", "Option Dialog"), wspectrum2);
  ...
  labelspinpfsize = new QLabel(tr("Schriftgröße", "Option Dialog"), groupspinpfsize);
  ...
  wmprezision->setText(tr("Präzision 2", "Option Dialog"));
...
void odlg::setdaten(TGrunddaten agrunddaten) {
  ...
  s = QString::asprintf("%1.0f", ogrunddaten.kalibrieranfang);
  eanfang->setText(s);
  s = QString::asprintf("%1.0f", ogrunddaten.kalibrierende);
  eende->setText(s);
  s = QString::asprintf("%9.0f", ogrunddaten.ddstakt);
  eddstakt->setText(s);
  s = QString::asprintf("%9.0f", ogrunddaten.maxwobbel);
  wobmax->setText(s);
  s = QString::asprintf("%i", ogrunddaten.frqfaktor);
  ...
  // SA
  s = QString::asprintf("%0.0f", ogrunddaten.frqa1);
  efrqa1->setText(s);
  s = QString::asprintf("%0.0f", ogrunddaten.frqb1);
  efrqb1->setText(s);
  s = QString::asprintf("%0.0f", ogrunddaten.frqa2);
  efrqa2->setText(s);
  s = QString::asprintf("%0.0f", ogrunddaten.frqb2);
  efrqb2->setText(s);
  s = QString::asprintf("%0.0f", ogrunddaten.frqzf1);
  efrqzf1->setText(s);
  s = QString::asprintf("%0.0f", ogrunddaten.frqzf2);
  efrqzf2->setText(s);
  s = QString::asprintf("%0.0f", ogrunddaten.frqshift);
  efrqshift->setText(s);
  s = QString::asprintf("%0.2f", ogrunddaten.psavabs1);
  eabsolut1->setText(s);
  s = QString::asprintf("%0.2f", ogrunddaten.psavabs2);
  eabsolut2->setText(s);
  s = QString::asprintf("%0.2f", ogrunddaten.psavabs3);
  eabsolut3->setText(s);
  s = QString::asprintf("%0.2f", ogrunddaten.psav300);
  eb300->setText(s);
  s = QString::asprintf("%0.2f", ogrunddaten.psav7k);
  eb7k->setText(s);
  s = QString::asprintf("%0.2f", ogrunddaten.psav30k);
  eb30k->setText(s);

  s = QString::asprintf("%0.0f", ogrunddaten.bw300_max);
  egsab300max->setText(s);
  s = QString::asprintf("%0.0f", ogrunddaten.bw300_min);
  egsab300min->setText(s);
  s = QString::asprintf("%0.0f", ogrunddaten.bw7kHz_max);
  egsab7kmax->setText(s);
  s = QString::asprintf("%0.0f", ogrunddaten.bw7kHz_min);
  egsab7kmin->setText(s);
  s = QString::asprintf("%0.0f", ogrunddaten.bw30kHz_max);
  egsab30kmax->setText(s);
  s = QString::asprintf("%0.0f", ogrunddaten.bw30kHz_min);
  egsab30kmin->setText(s);

  s = QString::asprintf("%i", ogrunddaten.safehlermax);
  esafehlermax->setText(s);
  s = QString::asprintf("%i", ogrunddaten.safehlermin);
  esafehlermin->setText(s);
  s = QString::asprintf("%i", ogrunddaten.saminschritte);
...
void odlg::tip(bool btip) {
  QString tip_einkanalig = tr(
      ...
      "Messeingang. Durch deaktivieren wird der zweite Kanal aktiviert.<br>", "tooltip option text");
  QString tip_mathkorr = tr(
      "<b>Mathematische Glättung der SWV-Kurve</B><BR>"
      "Bei den geringsten dB-Änderungen ändert sich das SWV in grossen<br> "
      "Schritten. Aber nur bei grossem SWV. Um das Zickzack in der<br> "
      "Darstellung etwas zu glätten wird aus 3 Messwerten der Mittelwert<br> "
      "gebildet. Das gleiche math. Verfahren wird beim Erzeugen der<br> "
      "SWV-Kalibrierdatei angewendet.<br>", "tooltip option text");
  QString tip_defdatei = tr(
      "<b>Defaultdateiname der Sondendatei</B><BR>"
      "Dieser Dateiname wird als default-Einstellung für die Sondendatei<br> "
      "benutzt. Der Dateiname wird ohne \"EXT\" angegeben. Das \"EXT\" wird<br> "
      "vom der SW automatisch ergänzt.<br> ", "tooltip option text");
  QString tip_fmaxwobbel = tr(
      ...
      "<em>NWT500</em> mit DDS-Takt 1200MHz \tetwa 500MHz<br> ", "tooltip option text");
  QString tip_serielle = tr(
      ...
      "COM1 bis COM256 auswählbar.<br> ", "tooltip option text");
  QString tip_pll = tr(
      ...
      "voreingestellt. Wird diese Einstellmöglichkeit aktiviert<br> "
      "ist eine andere FW im PIC zu installieren.<br> ", "tooltip option text");
  QString tip_ddstakt = tr(
      ...
      "Jetzt müsste die die VFO-Frequenz genau 10MHz betragen", "tooltip option text");
  QString tip_kalibrierend = tr(
      ...
      "Das ist erforderlich um die Anzahl der Frequenzstützpunkte<br> "
      "für die mathematische Kalibrierkorrektur festzulegen.<br> "
      "Die zweite Funktion dieser Endfrequenz ist bei der Frequenzlupe<br>. "
      "Wird immer auf \"minus\" gedrückt muss es eine obere Grenze geben.<br> "
      "Dazu wird diese Einstellung benutzt.<br>", "tooltip option text");
  QString tip_kalibrieranf = tr(
      ...
      "Wird immer auf \"minus\" gedrückt muss es eine untere Grenze geben.<br> "
      "Dazu wird diese Einstellung benutzt.<br>", "tooltip option text");
  QString tip_color = tr(
      "<b>Einstellung der Farbe</B><BR>"
      "Die Farbwerte werden in der \"*.hfc\" Datei<br>"
      "abgespeichert.<br>", "tooltip option text");
  QString tip_daempfung = tr(
      "<b>Aktivierung des FA-Dämpfungsgliedes</B><BR>"
      "In der Grund-SW wird ein Dämpfungsglied <br>"
      "10,20,20 dB unterstuetzt. Nach Aktivierung <br>"
      " wird das Dämpfungsglied vom FA unterstuetzt. <br>"
      "Voraussetzung ist die FW ab 1.10 <br>"
      "<em>Bitte beim alten Dämpfungsglied die <br>"
      "neue Beschaltung beachten.</em>", "tooltip option text");
  QString tip_hwrueck = tr(
      "<b>Rückmeldung Spektrumanalysator Software</B><BR>"
      ...
      "zur SW zurückgemeldet. Voraussetzung ist eine  <br>"
      "entsprechende FW im NWT-FA. Das funktioniert nur <br>"
      "mit dem NWT vom Funkamateur", "tooltip option SAV text");
  QString tip_autoschritt = tr(
      "<b>Automatische Schrittkorrektur</B><BR>"
      "Ist die HW-Rückmeldung aktiv kann mit diesem Punkt <br>"
      "die Schrittweite automatisch angepasst werden. Beträgt <br>"
      "z.B.: die Schrittweite im NWT 10kHz und am SAV ist 7kHz <br>"
      "eingestellt würde das zu Fehlmessungen führen. Ist der <br>"
      "Punkt aktiv, wird die Schrittzahl so weit erhöht, das die <br>"
      "Schrittweite kleiner 3,5kHz ist. Ist das nicht möglich <br>"
      "kommt ein Warnhinweis", "tooltip option SAV text");
  QString tip_sabereich1 = tr(
      "<b>SA Bereich1/3 Einstellungen</B><BR>"
      "Hier werden minimale, maximale <br>"
      "Frequenz und die ZF für den  <br>"
      "Spektrumanalyservorsatz vom FA <br>"
      "eingegeben. Die maximale Frequenz  <br>"
      "ergibt sich aus der ersten ZF des SA. <br>"
      "\"Shift\" ist für \"Spek.FRQ-shift\" <br>"
      "Verschiebung der angezeigten Frequenz <br>"
      "im Display.", "tooltip option SAV text");
  QString tip_sabereich2 = tr(
      "<b>SA Bereich2 Einstellungen</B><BR>"
      "Hier werden minimale, maximale Frequenz und die ZF <br>"
      "für den Spektrumanalyservorsatz vom FA eingegeben <br>"
      "Die minimale Frequenz ergibt sich aus der ersten ZF des SA. <br>"
      "Die VFO-Frequenz ist hier unterhalb der ersten ZF", "tooltip option SAV text");
  QString tip_sastandart = tr(
      "<b>Spektrumanalysator Standartwerte</B><BR>"
      "Wurde falsche Frequenzwerte eingegeben. Kann mann alles <br>"
      "wieder auf Standartwerte einstellen. ", "tooltip option SAV text");
  QString tip_sapegelabs = tr(
      "<b>Spektrumanalysator Kalibrierung</B><BR>"
      "Den Pegel Bereich 1/2/3 verschieben <br>", "tooltip option SAV text");
  QString tip_sapegelbw = tr(
      "<b>Spektrumanalysator Kalibrierung</B><BR>"
      "Die verschiedenen Pegel der Bandweiten angleichen <br>", "tooltip option SAV text");
  QString tip_sacalnull = tr(
      "<b>Spektrumanalysator Kalibrierung</B><BR>"
      "Das Kalibrierungaarray auf 0,0 setzen. <br>", "tooltip option SAV text");
  QString tip_sacal = tr(
      "<b>Spektrumanalysator Kalibrierung</B><BR>"
      "Das Kalibrierungaarray mit Hilfe der <br>"
      "Kurvendateien optimieren. Der Dämpfungsverlauf <br>"
      "des Tiefpass Bereich1 und des Bandpasses Bereich2 <br>"
      "wird mit in die Kalibrieung genommen. Siehe Beschreibung. <br>", "tooltip option SAV text");
  QString tip_sabwunten = tr(
      "<b>Spektrumanalysator automatischer Schrittkorrektur</B><BR>"
      "Hier wird die Frequenzgrenze eingetragen, bei der <br>"
      "die Schrittanzahl wieder automatisch verringert wird. <br>"
      "Dieser wert muss kleiner sein als der \"max Wert\".", "tooltip option SAV text");
  QString tip_sabwoben = tr(
      "<b>Spektrumanalysator automatischer Schrittkorrektur</B><BR>"
      "Hier wird die Frequenzgrenze eingetragen, bei <br>"
      "die Schrittweite durch Erhöhung der Schrittanzahl <br>"
      "korrigiert wird. Der Wert muss so gross gewählt <br>"
      "werden, dass mindestens 2 Messwerte innerhalb <br>"
      "der 3 dB Durchlasskurve entstehen. Bei 300Hz <br>"
      "Bandweite müsste also 100 Hz eingetragen werden.", "tooltip option SAV text");
  QString tip_saungenau = tr(
      "<b>Spektrumanalysator Kalibrierung</B><BR>"
      "Die graue Einfärbung der ungenauen <br>"
      "Messbereiche des SAV. <br>", "tooltip option SAV text");
  QString tip_sawarnung = tr(
      "<b>Spektrumanalysator Warnungen</B><BR>"
      "Ist die automatische Schrittkorrektur <br>"
      "deaktiviert und die Schrittweite zu <br>"
      "gross erfolgt ein Hinweis. Dieser <br>"
      "Warnung kann hier unterdrückt werden.", "tooltip option SAV text");
  QString tip_sawarnungmax = tr(
      ...
      "Hinweis störend, kann er hier <br>"
      "unterdrückt werden.", "tooltip option SAV text");
  QString tip_saminschritt = tr(
      "<b>Spektrumanalysator Kalibrierung</B><BR>"
      "Die unterste Grenze der Schritte bei <br>"
      "\"automatischer Schrittkorrektur\" <br>", "tooltip option SAV text");
  QString tip_schriftgroesse = tr(
      "<b>Einstellung der Schriftgröße</B><BR>"
      "Die Schriftgröße kann angepasst werden. <br>"
      "Es wirkt auf fast alle Texte im Programm. ", "tooltip option allgemein text");
  QString tip_focusumschaltung = tr(
      "<b>Automatische Focusumschaltung</B><BR>"
      "Durch die Herauslösung der grafischen Darstellung <br>"
      "ist die Bedienung des Programmes etwas schwieriger. <br>"
      "Eine Vereinfachung ist die automatische Fokusübergabe <br>"
      "nach dem Betätigen eines Buttons. Welcher Button <br>"
      "das bewirken soll, wird hier festgelegt", "tooltip option allgemein text");
  QString tip_nfaudio = tr(
      "<b>Wobbel NF-Audio</B><BR>"
      ...
      "ist die Sensibilität mit der auf Pegelschwankungen <br>"
      "geachtet wird. Sobald eine Pegelschwankung erkannt wird, <br>"
      "werden die nächsten 5 Messpunkte ganz langsam gewobbelt. <br>"
      ...
      "99 = am unempfindlichsten <br>", "tooltip option allgemein text");
  QString tip_prezision = tr(
      "<b>Wattmeter</B><BR>"
      "2 Nachkommastellen bei der dBm-Anzeige <br>", "tooltip option allgemein text");
...
void odlg::checkboxsastatus_checked(bool check_ok) {
  if (check_ok) {
...
void odlg::sacal() {
  ...
  if (!s.isNull()) {
    ...
    if (f.open(QFile::ReadOnly)) {
      ...
      switch (i) {
        case 0:w.ebetriebsart = ewobbeln;
          w.linear1 = false;
          w.linear2 = false;
          break;
        case 1:
      ...
      QString s1, s2, s3, s4;
      s1 = QString::asprintf(" %fdB", pegel_0);
      s2 = QString::asprintf(" %i", j);
      s3 = QString::asprintf(" %fdB", pegelmin);
      s4 = QString::asprintf(" %fdB", pegelmax);
      QMessageBox::information(this, tr("SAV Kalibrierdatei", "Kalibrier-Kurvendatei laden"),
                               tr("<B>Die Daten wurden eingelesen und ergänzt</B><BR><BR>",
                                  "Kalibrier-Kurvendatei laden") +
                                   tr("Durchschnittlicher Pegel", "Kalibrier-Kurvendatei laden") + s1 + " <BR>" +
                                   tr("Anzahl der benutzten Messpunkte", "Kalibrier-Kurvendatei laden") + s2 +
                                   " <BR>" +
                                   tr("Pegelabweichung vom Durchschnitt", "Kalibrier-Kurvendatei laden") + s3 +
                                   " <BR>" +
                                   tr("Pegelabweichung vom Durchschnitt", "Kalibrier-Kurvendatei laden") + s4 +
                                   " <BR><BR>" +
                                   tr("Die Daten werden in die Datei \"sav.cal\" abgespeichert!",
                                      "Kalibrier-Kurvendatei laden"));
