...
#pragma ide diagnostic ignored "OCDFAInspection" // TODO Refactor
winsinfo::winsinfo(QWidget *parent, const char *name, bool modal) : QDialog(parent) {
  this->setModal(true);
  ...
  labelfontsize = new QLabel(tr("Schriftgröße"), ginfo);
...
WidgetWobbeln::WidgetWobbeln(QWidget *parent) : QWidget(parent) {
  ...
  popupmenu->addSeparator();
  akabellaenge = new QAction(tr("Ermitteln der geometrischen Kabellänge", "PopupMenue"), popupmenu);
...
void WidgetWobbeln::kabeldaten() {
  ...
  QString beschr(tr("<b>Ermittlung der Kabeldaten</b><br>"
                    ...
                    "<BR><BR>"
                    "Gemessene Länge des Kabels (m):", "Inputdialog in der Grafik"));
  QString qs, qv, qe;
  double v, e;
  bool ok;
  double l = QInputDialog::getDouble(this, tr("Kabellänge", "Inputdialog in der Grafik"),
                                     beschr, 0.0, 0, 10000, 3, &ok);
  if (ok) {
    v = 0.0133 * l * (mousefrequenz / 1000000.0);
    e = 1 / (v * v);
    qv = QString::asprintf("%1.3f", v);
    qe = QString::asprintf("%1.3f", e);
    qs = tr("<B>Errechnung der Kabeldaten</B><BR>"
            "Verkürzungsfaktor: ", "InfoBox in der Grafik") +
        qv +
        "<BR>" +
        tr("Dielektrizitätszahl: ", "InfoBox in der Grafik") +
        qe;
...
#ifdef LDEBUG
  qDebug("WidgetWobbeln::Kabellänge()");
#endif
  QString beschr(tr("<b>Ermittlung der Kabellänge</b><br>"
                    ...
                    "Resonanzfrequenz. Bei größeren Kabellängen ist es erforderlich<br> "
                    "die Endfrequenz zu verringern um genaue Messergebnisse zu erhalten"
                    "<br><br>"
                    "Verkürzungsfaktor des Kabels (Vorgabe = PE-Kabel):", "InputDialog in der Grafik"));
  QString qs, qer;
  double er;
  bool ok;
  double v = QInputDialog::getDouble(this, tr("Verkürzungsfaktor", "InputDialog in der Grafik"),
                                     beschr, 0.67, -10000, 10000, 3, &ok);
  if (ok) {
    er = v / (mousefrequenz / 1000000.0) * 75.0;
    qer = QString::asprintf("%1.3f", er);
    qs = tr("<B>Errechnung der Kabellänge</B><BR>"
            "Geometrische Länge des Kabels: ", "InfoBox in der Grafik") +
        qer +
        " m";
    QMessageBox::warning(this, tr("Geometrische Länge", "InfoBox in der Grafik"), qs);
  }
}
...
void WidgetWobbeln::mouseMoveEvent(QMouseEvent *e) {
  ...
  if (wobbeldaten.mess.daten_enable) {
    mouseposx[kursorix] = e->position().x();
    mouseposy[kursorix] = e->position().y();
  ...
  if (drawenable) {
    ...
    if ((wobbeldaten.ebetriebsart == espektrumanalyser) or
      ...
      if (wobbeldaten.linear1 or wobbeldaten.linear2) {
        ...
        h4 = h5 - ObererRand;
        //Test ob obere Abdunklung größer 5 dB eingeblendet werden soll
...
void WidgetWobbeln::refreshPixmap() {
  ...
  //Groesse anpassen und Pixmap löschen
  setParameterPixmap();
  QPainter p;
  p.begin(&pix);
...
void WidgetWobbeln::paintEvent(QPaintEvent *) {
  ...
  //zeichen die Groesse hat sich geändert
  ...
  switch (wobbeldaten.ebetriebsart) {
    case eantdiagramm:
      ...
      for (i = idbmax, j = 0; i >= idbmin; i -= antdia_db_teilung, j++) {
        p->setPen(penhline);
        p->drawEllipse(LinkerRand + j * w10db, ObererRand + j * w10db, durchmesser - j * w10db * 2,
                       durchmesser - j * w10db * 2);
        s = QString::asprintf("%idB", i);
        p->setPen(penblacksolid);
        p->drawText(LinkerRand + durchmesser / 2 - 15, ObererRand + j * w10db - 3, s);
        //in der Mitte nur einmal Beschriften
        if (i != idbmin)
          p->drawText(LinkerRand + durchmesser / 2 - 15, ObererRand + durchmesser - j * w10db + 13, s);
      }
      // Gradlinien
      p->setPen(penhline);
      for (i = 0; i <= 360; i += 10) {
    ...
    case ewobbeln:
      if (!wobbeldaten.linear1 && !wobbeldaten.linear2) { // logaritmische Beschriftung
        ...
        for (i = int(ymax / 10.0), j = 0; i >= int(ymin / 10.0); i--, j++) {
          ...
          s = QString::asprintf("%3i", (i - displayshift) * 10);
        ...
        if (wobbeldaten.hline != 0.0) {
          ...
          s = QString::asprintf("%0.2fdB", wobbeldaten.hline);
        ...
        for (i = 0; i < (ydisplayteilung_lin + 1); i++) {
          ...
          bx = 20.0 * log10(bx);
          if (bx > -10.0) {
            s = QString::asprintf("%1.2f", bx);
          } else {
            s = QString::asprintf("%1.1f", bx);
          }
          if (bx < -99.9) {
          ...
          //lineare Beschriftung
          s = QString::asprintf("%1.1f", double((11 - i) * 0.1));
    ...
    case eswrant:
      ...
      for (i = 0; i < (yswrteilung * 10 + 1); i++) {
        ...
        if ((i % xmodulo) == 0) {
          ...
          QString::asprintf("%1.1f", double(((yswrteilung * 10 - i) * 0.1)) + 1.0);
          //an den linken Rand schreiben
          p->setPen(penblacksolid);
          p->drawText(1, int(round(py1 * i)) + ObererRand + 3, s);
          //SWV Beschriftung
          s = QString::asprintf("%1.1f", double(((yswrteilung * 10 - i) * 0.1)) + 1.0);
          p->setPen(penblacksolid);
          p->drawText(int(round(px2)) + 3, int(round(py1 * i)) + ObererRand + 3, s);
          p->setPen(penlightgraydot);
          p->drawLine(int(round(px1)), int(round(py1 * i)) + ObererRand,
...
//        qDebug(((i+1) % xmodulo));
  ...
  if (wobbeldaten.ebetriebsart != eantdiagramm) {
    ...
    while ((xRaster * i) < wxende) {
      ...
      //je nach Länge des Strings den Text etwas nach links schieben
      QLocale::setDefault(QLocale(QLocale::German, QLocale::Germany));
      //Grundeinstellung
      switch (frqbereich) {
        case hz:xshift = 10;
          s = QString::asprintf("%3.0f", xRaster * i / me);
          break;
        case khz:xshift = 10;
          s = QString::asprintf("%3.1f", xRaster * i / me);
          if ((xRaster) < 1000.0) {
            // Bereich kleiner 1MHz
            xshift = 15;//Zahl 30px weiter links
            s = QString::asprintf("%3.2f", xRaster * i / me);
          }
          if ((xRaster) < 100.0) {
            // Bereich kleiner 100kHz
            xshift = 20;//Zahl 30px weiter links
            s = QString::asprintf("%3.3f", xRaster * i / me);
          }
          if ((xRaster) < 10.0) {
            // Bereich kleiner 10kHz
            xshift = 25;//Zahl 30px weiter links
            s = QString::asprintf("%3.4f", xRaster * i / me);
          }
          break;
        case mhz:xshift = 10;
          s = QString::asprintf("%3.1f", xRaster * i / me);
          ...
          if ((xRaster) < 1000.0) {
            ...
            s = QString::asprintf("%3.5f", xRaster * i / me);
          }
          if ((xRaster) < 100.0) {
            // Bereich kleiner 100Hz
            xshift = 35;//Zahl 30px weiter links
            s = QString::asprintf("%3.6f", xRaster * i / me);
          }
          break;
        case ghz:s = QString::asprintf("%3.1f", xRaster * i / me);
          ...
          if ((xRaster) < 10000.0) {
            ...
            s = QString::asprintf("%3.7f", xRaster * i / me);
          }
          if ((xRaster) < 1000.0) {
            // Bereich kleiner 1kHz
            xshift = 45;//Zahl 30px weiter links
            s = QString::asprintf("%3.8f", xRaster * i / me);
          }
          if ((xRaster) < 100.0) {
            // Bereich kleiner 100Hz
            xshift = 50;//Zahl 30px weiter links
            s = QString::asprintf("%3.9f", xRaster * i / me);
          }
          break;
      }
      if (boolrot) {
    ...
    s = QString::asprintf(" %1.1f ", xRaster);
    s = qstr + s + xRasterStr;
    if (boolrot) {
      p->setPen(penredsolid);
    } else {
...
//*************************************************************************************
  ...
  if (wobbeldaten.mess.daten_enable) {
    ...
    //##########################################################
    // alle Listen löschen aber nur wenn es die Onlinekurve ist und kein
    // Hintergrund
    ...
    switch (wobbeldaten.ebetriebsart) {
      case eantdiagramm:
        ...
        for (i = adbegin; i <= adend; i++) {//von Spannung als AD-Wert für unterstes Gradmass
          //bis Spannung als AD-Wert für oberstes Gradmass
            ...
            for (k = 0; k < 5; k++) {
              if (mouseposx1[k] != mouseposx[k]) {
                ...
                */
                ...
                if ((wobbeldaten.ebetriebsart == espektrumanalyser) or
                    (wobbeldaten.ebetriebsart == espekdisplayshift)) {
                  py1 = korrsavpegel(py1) -
                      getsavcalwert(afrequenz + wobbeldaten.schrittfrequenz * kxm1[k]);
                }
                py1 = py1 * yzeichenfaktor_log + yzeichenshift_log;
              ...
              if (mouseposx1[k] != mouseposx[k]) {
                ...
                */
                ...
                if ((wobbeldaten.ebetriebsart == espektrumanalyser) or
                    (wobbeldaten.ebetriebsart == espekdisplayshift)) {
                  py1 = korrsavpegel(py1) -
                      getsavcalwert(afrequenz + wobbeldaten.schrittfrequenz * kxm2[k]);
                }
                py1 = py1 * yzeichenfaktor_log + yzeichenshift_log;
        ...
        if (!wobbeldaten.bhintergrund) {
          ...
          for (k = 0; k < 5; k++) {
            ...
            if ((mouseposx[k] != 0) && (mouseposy[k] != 0) && bmouse) { //Cursorkreuz aktiv
              ...
              s = QString::asprintf("%2.6f MHz", px1 / 1000000.0);
              emit multieditinsert(s);
              //##########################################################
              // Kursortext Kanal 1
              //##########################################################
              if (wobbeldaten.bkanal1) {
                ...
                if (wobbeldaten.linear1) {
                  py1 = (100.0 + py1) / 100.0;
                  bx = 20.0 * log10(py1);
                  s = QString::asprintf(" %1.2f, %1.2fdB", py1, bx);
                  s = qstr + s;
                } else {
          ...
          if (wobbeldaten.bkanal1) {
            ...
            if (wobbeldaten.linear1) {
              ...
              bx = 20.0 * log10(py1);
              s = QString::asprintf("%1.2f, %1.2fdB %2.6fMHz", py1, bx, px1);
              squdaten = QString::asprintf("%6.0f;%1.2fdB", px1 * 1000000.0, bx);
            } else {
              if ((wobbeldaten.ebetriebsart == espektrumanalyser) or
                  (wobbeldaten.ebetriebsart == espekdisplayshift)) {
                s = QString::asprintf("%2.2fdBm %2.6fMHz", maxdbk1 + wobbeldaten.dbshift1 + displayshift, px1);
              } else {
                s = QString::asprintf("%2.2fdB %2.6fMHz", maxdbk1 + wobbeldaten.dbshift1 + displayshift, px1);
              }
            }
            s = qstr + s;
            emit multieditinsert(s);
            ...
            if (wobbeldaten.linear1) {
              ...
              if ((wobbeldaten.ebetriebsart == espektrumanalyser) or
                  (wobbeldaten.ebetriebsart == espekdisplayshift)) {
                s = QString::asprintf("%2.2fdBm %2.6fMHz", mindbk1 + wobbeldaten.dbshift1 + displayshift, px1);
              } else {
                s = QString::asprintf("%2.2fdB %2.6fMHz", mindbk1 + wobbeldaten.dbshift1 + displayshift, px1);
              }
            }
            s = qstr + s;
            emit multieditinsert(s);
            ...
            if (wobbeldaten.bandbreite3db) {
              ...
              if ((db3xa != 0.0) and (db3xb != 0.0)) {
                ...
                s = QString::asprintf(";%1.3f", bandbreite3db);
                squdaten = squdaten + s;
                qstr = tr("B3db:", "im Infofenster");
                s = QString::asprintf(" %1.3f Hz", bandbreite3db);
                if (bandbreite3db > 1000.0) {
                  s = QString::asprintf(" %1.3f kHz", bandbreite3db / 1000.0);
                }
                if (bandbreite3db > 1000000.0) {
                  s = QString::asprintf(" %1.3f MHz", bandbreite3db / 1000000.0);
                }
                s = qstr + s;
                emit multieditinsert(s);
                s = QString::asprintf("Q: %1.2f", ((db3xa + db3xb) / 2.0) / bandbreite3db);
                emit multieditinsert(s);
                ...
                //untere Frequenz
                s = QString::asprintf("f1: %1.6f MHz", db3xa / 1000000.0);
                emit multieditinsert(s);
                //mitten Frequenz
                s = QString::asprintf("fm: %1.6f MHz", (db3xa + ((db3xb - db3xa) / 2)) / 1000000.0);
                emit multieditinsert(s);
                //obere Frequenz
                s = QString::asprintf("f2: %1.6f MHz", db3xb / 1000000.0);
                emit multieditinsert(s);
              } else {
              ...
              if (wobbeldaten.binvers) {
                if ((db3xai != 0.0) and (db3xbi != 0.0)) {
                  ...
                  s = QString::asprintf(" %1.2f", ((db3xai + db3xbi) / 2.0) / bandbreite3db);
                  s = qstr + s;
                  emit multieditinsert(s);
                  if (wobbeldaten.bandbreitegrafik) {
                    p->setPen(penreddot);
                  ...
                  s = QString::asprintf("f1: %1.6f MHz", db3xai / 1000000.0);
                  emit multieditinsert(s);
                  s = QString::asprintf("f2: %1.6f MHz", db3xbi / 1000000.0);
                  emit multieditinsert(s);
                } else {
            ...
            if (wobbeldaten.bandbreite6db) {
              ...
              if ((db3xa != 0.0) and (db3xb != 0.0)) {
                ...
                s = QString::asprintf(" %1.3f Hz", bandbreite6db);
                if (bandbreite3db > 1000.0) {
                  s = QString::asprintf(" %1.3f kHz", bandbreite6db / 1000.0);
                }
                if (bandbreite3db > 1000000.0) {
                  s = QString::asprintf(" %1.3f MHz", bandbreite6db / 1000000.0);
                }
                s = qstr + s;
                emit multieditinsert(s);
                if (wobbeldaten.bandbreitegrafik) {
            ...
            if (wobbeldaten.bandbreite6db) {
              ...
              if ((db3xa != 0.0) and (db3xb != 0.0)) {
                ...
                s = QString::asprintf("f2: %1.6f MHz", db3xb / 1000000.0);
                emit multieditinsert(s);
                s = QString::asprintf("Shape-Faktor: %1.6f", bandbreite3db / bandbreite6db);
                emit multieditinsert(s);
              } else {
                s = "B60db: -";
                emit multieditinsert(s);
          ...
          if (wobbeldaten.bkanal2) {
            ...
            if (wobbeldaten.linear2) {
              ...
              if ((wobbeldaten.ebetriebsart == espektrumanalyser) or
                  (wobbeldaten.ebetriebsart == espekdisplayshift)) {
                s = QString::asprintf("%2.2fdBm %2.6fMHz", maxdbk2 + wobbeldaten.dbshift2 + displayshift, px1);
              } else {
                s = QString::asprintf("%2.2fdB %2.6fMHz", maxdbk2 + wobbeldaten.dbshift2 + displayshift, px1);
              }
            }
            s = qstr + s;
            emit multieditinsert(s);
            ...
            if (wobbeldaten.linear2) {
              ...
              if ((wobbeldaten.ebetriebsart == espektrumanalyser) or
                  (wobbeldaten.ebetriebsart == espekdisplayshift)) {
                s = QString::asprintf("%2.2fdBm %2.6fMHz", mindbk2 + wobbeldaten.dbshift2 + displayshift, px1);
              } else {
                s = QString::asprintf("%2.2fdB %2.6fMHz", mindbk2 + wobbeldaten.dbshift2 + displayshift, px1);
              }
            }
            s = qstr + s;
            emit multieditinsert(s);
            //3dB Bandbreite + Q für Kanal 2
            if (wobbeldaten.bandbreite3db) {
              db3xa = 0.0;
              db3xb = 0.0;
              ...
              if ((db3xa != 0.0) and (db3xb != 0.0)) {
                ...
                qstr = tr("B3db:", "im Infofenster");
                s = QString::asprintf(" %1.3f Hz", bandbreite3db);
                if (bandbreite3db > 1000.0)s = QString::asprintf(" %1.3f kHz", bandbreite3db / 1000.0);
                if (bandbreite3db > 1000000.0)s = QString::asprintf(" %1.3f MHz", bandbreite3db / 1000000.0);
                s = qstr + s;
                emit multieditinsert(s);
                s = QString::asprintf("Q: %1.2f", ((db3xa + db3xb) / 2.0) / bandbreite3db);
                emit multieditinsert(s);
            ...
            if (wobbeldaten.bandbreite6db) {
              ...
              if ((db3xa != 0.0) and (db3xb != 0.0)) {
                ...
                qstr = tr("B6db:", "im Infofenster");
                s = QString::asprintf(" %1.3f Hz", bandbreite6db);
                if (bandbreite3db > 1000.0)s = QString::asprintf(" %1.3f kHz", bandbreite6db / 1000.0);
                if (bandbreite3db > 1000000.0)s = QString::asprintf(" %1.3f MHz", bandbreite6db / 1000000.0);
                s = qstr + s;
                emit multieditinsert(s);
                if (wobbeldaten.bandbreitegrafik) {
            ...
            if (wobbeldaten.bandbreite6db) {
              ...
              if ((!wobbeldaten.linear2)) {
                if ((db3xa != 0.0) and (db3xb != 0.0)) {
                  ...
                  s = QString::asprintf(" %1.6f", bandbreite3db / bandbreite6db);
                  s = qstr + s;
                  emit multieditinsert(s);
                } else {
            ...
            for (k = 0; k < 5; k++) {
              if (mouseposx1[k] != mouseposx[k]) {
                ...
                if (wobbeldaten.bswrkanal2) {
                  ...
                  py1 = w2 * y2faktor + y2shift;
                  py1 = py1 - getswrkalibrierwertk2(
                      afrequenz + wobbeldaten.schrittfrequenz * double(kx2[k]));
                } else {
                  w1 = double(wobbeldaten.mess.k1[kx1[k]]);
                  w2 = double(wobbeldaten.mess.k1[kx2[k]]);
                  w3 = double(wobbeldaten.mess.k1[kx3[k]]);
                  py1 = w2 * y1faktor + y1shift;
                  py1 = py1 - getswrkalibrierwertk1(
                      afrequenz + wobbeldaten.schrittfrequenz * double(kx2[k]));
                }
                if (py1 > -1.0)py1 = -1.0;
                uricht = pow(10.0, (py1 / 20.0));
                ...
                if (wobbeldaten.bswriteration) {
                  if (wobbeldaten.bswrkanal2) {
                    py1 = w1 * y2faktor + y2shift;
                    py1 = py1 - getswrkalibrierwertk2(
                        afrequenz + wobbeldaten.schrittfrequenz * double(kx1[k]));
                  } else {
                    py1 = w1 * y1faktor + y1shift;
                    py1 = py1 - getswrkalibrierwertk1(
                        afrequenz + wobbeldaten.schrittfrequenz * double(kx1[k]));
                  ...
                  if (wobbeldaten.bswrkanal2) {
                    py1 = w3 * y2faktor + y2shift;
                    py1 = py1 - getswrkalibrierwertk2(
                        afrequenz + wobbeldaten.schrittfrequenz * double(kx3[k]));
                  } else {
                    py1 = w3 * y1faktor + y1shift;
                    py1 = py1 - getswrkalibrierwertk1(
                        afrequenz + wobbeldaten.schrittfrequenz * double(kx3[k]));
                ...
                if ((wobbeldaten.ebetriebsart == eswrneu) or (wobbeldaten.ebetriebsart == eswrant)) {
                  my1[k] = int(round(hoehe - round((swv - 1.0) * (double(hoehe) / yswrteilung)) - 1 +
                      ObererRand));
                }
                if (wobbeldaten.ebetriebsart == eimpedanz) {
                  swv = (50.0 * swv) - 50.0;
                  //            swv = 10.0;
                  my1[k] = int(round(hoehe - round((swv - 1.0) * (double(hoehe) / 200.0)) - 2 +
                      ObererRand));
                }
                if (wobbeldaten.ebetriebsart == eswrant) {
                  a = a_100 * laenge / 100.0;
                  ...
                  s_ant = (1.0 + r_ant) / (1.0 - r_ant);
                  my2[k] = int(
                      round(hoehe - round((s_ant - 1.0) * (double(hoehe) / yswrteilung)) - 1 +
                          ObererRand));
                  if (my2[k] > (hoehe + ObererRand))my2[k] = hoehe + ObererRand;
                  if (my2[k] < ObererRand)my2[k] = ObererRand;
                }
          ...
          for (k = 0; k < 5; k++) {
            ...
            if ((mouseposx[k] != 0) && (mouseposy[k] != 0) && bmouse) { //Cursorkreuz aktiv
              ...
              if (wobbeldaten.bswrkanal2) {
                ...
                py1 = py1 -
                    getswrkalibrierwertk2(afrequenz + wobbeldaten.schrittfrequenz * double(kx2[k]));
              } else {
                py1 = kw2[k] * y1faktor + y1shift;
                py1 = py1 -
                    getswrkalibrierwertk1(afrequenz + wobbeldaten.schrittfrequenz * double(kx2[k]));
              }
              if (py1 > -1.0)py1 = -1.0;
              uricht = pow(10.0, (py1 / 20.0));
              swv = (1.0 + uricht) / (1.0 - uricht);
              ...
              if ((wobbeldaten.ebetriebsart == eswrneu) or (wobbeldaten.ebetriebsart == eswrant)) {
                ...
                s = QString::asprintf(" %4.4f", swv);
              }
              if (wobbeldaten.ebetriebsart == eimpedanz) {
                if (wobbeldaten.bswrkanal2) {
                  py1 = getswrkalibrierwertk2(
                      afrequenz + wobbeldaten.schrittfrequenz * double(kx1[k]));
                } else {
                  py1 = getswrkalibrierwertk1(
                      afrequenz + wobbeldaten.schrittfrequenz * double(kx1[k]));
                }
//              s = QString::asprintf("Eich: %4.4f dB", py1);
//              emit multieditinsert(s);
                swv = (50.0 * swv) - 50.0;
                qstr = tr("Z:", "im Infofenster");
                s = QString::asprintf(" %4.4f Ohm", swv);
              }
              s = qstr + s;
              emit multieditinsert(s);
          ...
          if ((wobbeldaten.ebetriebsart == eswrneu) or (wobbeldaten.ebetriebsart == eswrant)) {
            ...
            s = QString::asprintf(" %2.2f  %2.6f MHz", minswr, px1);
            s = qstr + s;
            emit multieditinsert(s);
            ...
            s = QString::asprintf(" %2.2f  %2.6f MHz", maxswr, px1);
            s = qstr + s;
            emit multieditinsert(s);
            s = "---------------------";
            emit multieditinsert(s);
          ...
          if (wobbeldaten.ebetriebsart == eimpedanz) {
            ...
            s = QString::asprintf(" %2.2f  %2.6f MHz", swv, px1);
            s = qstr + s;
            emit multieditinsert(s);
            // SWV maximum anzeigen
            px1 = (maxk1 * wobbeldaten.schrittfrequenz) + afrequenz;
...
QString WidgetWobbeln::kursorsavstr(double pegel) {
  ...
  if (wobbeldaten.bsavdbm) {
    s2 = QString::asprintf("%2.2fdBm", pegel);
    s1 = s1 + s2 + "; ";
  }
  if (wobbeldaten.bsavdbuv) {
    s2 = QString::asprintf("%2.2fdBuV", pegel + 106.9897);
    s1 = s1 + s2 + "; ";
  }
  if (wobbeldaten.bsavuv) {
    ...
    if (uvolt > 0.999) {
      s2 = QString::asprintf("%3.1f V", uvolt);
    }
    if (uvolt < 1.0) {
      uvolt = uvolt * 1000.0;
      s2 = QString::asprintf("%3.1f mV", uvolt);
    }
    if (uvolt < 1.0) {
      uvolt = uvolt * 1000.0;
      s2 = QString::asprintf("%3.1f uV", uvolt);
    }
    s1 = s1 + s2 + "; ";
  }
  if (wobbeldaten.bsavwatt) {
    ...
    if (power > 0.999) {
      s2 = QString::asprintf("%3.1f W", power);
      if (power > 999.999) {
        s2 = QString::asprintf("%3.2f kW", power / 1000.0);
      }
    }
    if (power < 1.0) {
      power = power * 1000.0;
      s2 = QString::asprintf("%3.1f mW", power);
    }
    if (power < 1.0) {
      power = power * 1000.0;
      s2 = QString::asprintf("%3.1f uW", power);
    }
    if (power < 1.0) {
      power = power * 1000.0;
      s2 = QString::asprintf("%3.1f nW", power);
    }
    if (power < 1.0) {
      power = power * 1000.0;
      s2 = QString::asprintf("%3.1f pW", power);
    }
    s1 = s1 + s2 + "; ";
  }
  s2 = s1;
...
void WidgetWobbeln::printdiagramm(const QFont &pfont, const QStringList &astrlist) {
  ...
  if (dialog.exec()) {
    ...
    p.drawText(xp, yp, qs);
    yp += zeilenabstand;
    p.drawLine(xp, yp, p.device()->width() - xp, yp);
    yp += zeilenabstand;

    qa = QString::asprintf("%3.6f", double(wobbeldaten.anfangsfrequenz) / 1000000.0);
    qe = QString::asprintf("%3.6f", double(((wobbeldaten.schritte - 1) * wobbeldaten.schrittfrequenz) +
        wobbeldaten.anfangsfrequenz) / 1000000.0);
    qw = QString::asprintf("%4.3f", double(wobbeldaten.schrittfrequenz) / 1000.0);
    qm = QString::asprintf("%4i", wobbeldaten.schritte);
    qz = QString::asprintf("%4i", wobbeldaten.ztime * 10);
...
//        qDebug("yp=%i xp=%i", yp, xp);
      ...
      yp += zeilenabstand;
      //Abschlusslinie
      //    p.drawLine(xp, yp, p_printer->width()-xp, yp);
    }
  }
}
...
#endif
  p_printer->setOutputFormat(QPrinter::PdfFormat);
  p_printer->setPageSize(QPageSize::A4);
  p_printer->setPageOrientation(QPageLayout::Orientation::Portrait);
...
void WidgetWobbeln::bildspeichern(const TBildinfo &bildinfo, const QStringList &astrlist) {
  ...
  if (ok) {
    ...
    s = QFileDialog::getSaveFileName(this, tr("Bild Speichern", "Grafik Filedialog"),
                                     imgpath.path(), "PNG (*.png *.PNG);;BMP (*.bmp *.BMP);;JPG (*.jpg *.JPG)",
                                     &qs1);
...
void WidgetWobbeln::keyPressEvent(QKeyEvent *event) {
  ...
  switch (event->key()) {
    ...
    case Qt::Key_Return:
      ...
      break;
    case Qt::Key_1:setWKursorNr(4); emit resetKursor(4);
      break;
    case Qt::Key_2:setWKursorNr(3); emit resetKursor(3);
      break;
    case Qt::Key_3:setWKursorNr(2); emit resetKursor(2);
      break;
    case Qt::Key_4:setWKursorNr(1); emit resetKursor(1);
      break;
    case Qt::Key_5:setWKursorNr(0); emit resetKursor(0);
      break;
    case Qt::Key_0:
      for (k = 0; k < 5; k++) {
        kursorix = k;
...
//      menmouse->hide();
      ...
      emit resetKursor(4);
      break;
    case Qt::Key_W:emit wobbelndauer();
      break;
    case Qt::Key_E:emit wobbelneinmal();
      break;
    case Qt::Key_S:emit wobbelnstop();
      break;
    case Qt::Key_F:CreateQuarzDatei();
      break;
    case Qt::Key_D:WriteToQuarzDatei(squdaten);
      break;
    case Qt::Key_C:CloseQuarzDatei();
      break;
